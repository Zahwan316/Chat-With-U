  ...
  return(
    <div className='w-full h-20 p-2 flex flex-row relative top-5  rounded-xl mb-2 hover:bg-[#5356FF] cursor-pointer transition-all' data-id={props.user_from_id} data-user-target-id={props.user_target_id} onClick={props.onClick}>
      ...
      <div className="flex flex-col justify-evenly" data-id={props.user_from_id}>
          <span data-id={props.user_from_id} data-user-target-id={props.user_target_id} className='z-0'>{props.time}</span>
          <span data-id={props.user_from_id} data-user-target-id={props.user_target_id} className="rounded-full bg-blue-500 min-w-6 w-auto flex justify-center items-center">{"999"}</span>
      </div>
    </div>
  )
})
